...
void mdbInitializeTypes(mdbDatatype **typetable)
{
  ...
  *typetable = (mdbDatatype*)malloc(MDB_TYPE_COUNT * sizeof(mdbDatatype));
  ...
  (*typetable)[0] = (mdbDatatype){ "INT-8",   5, 0, sizeof(byte),   &memcmp};
  (*typetable)[1] = (mdbDatatype){ "INT-16",  6, 0, sizeof(uint16), &memcmp};
  (*typetable)[2] = (mdbDatatype){ "INT-32",  6, 0, sizeof(uint32), &memcmp};
  (*typetable)[3] = (mdbDatatype){ "FLOAT",   5, 0, sizeof(float),
    &mdbCompareFloat};
  (*typetable)[4] = (mdbDatatype){ "CHAR-8",  6, 4, sizeof(byte),
    (CompareKeysPtr)&strncmp};
  (*typetable)[5] = (mdbDatatype){ "CHAR-16", 7, 4, sizeof(byte) * 2,
    &mdbCompareUnicode};
}
